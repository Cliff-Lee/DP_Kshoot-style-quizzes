...
import generated from './aaQuestionSeed.generated.json'
import { syllabusByCode } from './syllabus'
...
export const aaQuestionSeed:Question[]=rawQuestions.map(question=>{
  const point=syllabusByCode.get(question.syllabusCode.toUpperCase())
  if(!point)throw new Error(`Unknown generated syllabus code: ${question.syllabusCode}`)
  return {id:question.seedId,createdBy:'platform',visibility:'public',status:'approved',courseId:point.courseId,syllabusPointId:point.id,type:question.type,prompt:question.prompt,answerData:answerData(question),explanation:question.explanation,difficulty:question.difficulty,questionStyle:question.questionStyle,calculator:question.calculator,estimatedTimeSeconds:question.estimatedTimeSeconds,marksEstimate:question.marksEstimate,tags:['ib-aa',point.title.toLowerCase(),...question.tags],source:'platform_seed',options:options(question),createdAt:CREATED_AT,updatedAt:CREATED_AT}
})

export const aaCoverageQuestions=aaQuestionSeed.filter(question=>!question.id.includes('-extra-'))
export const representativeCoverageCodes=['SL 1.2','SL 2.11','SL 3.4','SL 4.6','SL 5.8'] as const
