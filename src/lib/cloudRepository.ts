...
export async function loadCloudWorkspace(): Promise<Omit<AppState,'user'|'activeCourseId'>> {
  ...
  const [questionsRes,classesRes,quizzesRes,sessionsRes,attemptsRes,batchesRes,usageRes] = await Promise.all([
    ...
    supabase.from('quiz_sessions').select('*,session_participants(*)').order('created_at',{ascending:false}).limit(30),
  ...
  ;[questionsRes,classesRes,quizzesRes,sessionsRes,attemptsRes,batchesRes,usageRes].forEach((res,i)=>warn(`load-${i}`,res.error))
...
export async function cloudCreateSession(item:QuizSession){if(!supabase)return;const {error}=await supabase.from('quiz_sessions').insert({id:item.id,quiz_id:item.quizId,teacher_id:item.teacherId,class_id:item.classId??null,pin:item.pin,status:item.status,current_question_index:item.currentQuestionIndex});warn('create-session',error)}
export async function cloudPatchSession(id:string,patch:Partial<QuizSession>){if(!supabase)return;const payload:any={};if(patch.status!==undefined)payload.status=patch.status;if(patch.currentQuestionIndex!==undefined)payload.current_question_index=patch.currentQuestionIndex;if(patch.revealedQuestionIndex!==undefined)payload.revealed_question_index=patch.revealedQuestionIndex;if(patch.questionStartedAt!==undefined)payload.question_started_at=patch.questionStartedAt;if(patch.startedAt!==undefined)payload.started_at=patch.startedAt;if(patch.endedAt!==undefined)payload.ended_at=patch.endedAt;const {error}=await supabase.from('quiz_sessions').update(payload).eq('id',id);warn('patch-session',error)}
...
export function subscribeToLiveSession(sessionId:string,onChange:()=>void){const client=supabase;if(!client)return()=>{};const channel=client.channel(`live-session:${sessionId}`).on('postgres_changes',{event:'*',schema:'public',table:'quiz_sessions',filter:`id=eq.${sessionId}`},onChange).on('postgres_changes',{event:'*',schema:'public',table:'session_participants',filter:`session_id=eq.${sessionId}`},onChange).on('postgres_changes',{event:'INSERT',schema:'public',table:'live_responses',filter:`session_id=eq.${sessionId}`},onChange).subscribe();return()=>{void client.removeChannel(channel)}}
