...
export function QuizBuilderPage(){
  ...
  const submit=(event:FormEvent)=>{event.preventDefault();if(!title.trim()||!selected.length)return;const quiz=actions.createQuiz({courseId:selectedCourseId,title:title.trim(),mode:deliveryMode,questionIds:selected,settings:{...DEFAULT_QUIZ_SETTINGS,calculatorAllowed:calculatorChoice!=='not_allowed',showLeaderboard,enablePowerups,enableStreakBonuses,showExplanations,timeLimitSeconds:timeLimit,pointsMode}});actions.logUsage('quiz_created');navigate(`/teacher/quizzes/${quiz.id}`)}
