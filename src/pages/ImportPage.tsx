...
export function ImportPage(){
  ...
  const importRows=async()=>{if(!result?.document||!result.courseFamily||result.globalErrors.length||!gate.allowed||saving)return;const rows=result.rows.filter(row=>row.valid&&(!row.duplicate||confirmDuplicates));const batchId=crypto.randomUUID();const records=rows.map(row=>({...importQuestionToRecord(row.question,state.user?.id??'teacher-demo',result.courseFamily),importBatchId:batchId,duplicateConfirmed:row.duplicate&&confirmDuplicates}));setSaving(true);setSaveError('');try{await actions.addQuestions(records);if(result.courseId)actions.setActiveCourse(result.courseId);const status=result.invalidCount?'partially_valid':'imported';actions.addImportBatch({id:batchId,createdAt:new Date().toISOString(),status,importedCount:records.length,errorCount:result.invalidCount,validationErrors:result.rows.filter(r=>r.errors.length).map(r=>({index:r.index,errors:r.errors}))},result.document);actions.logUsage('import_batch');setSaved(true)}catch(error){setSaveError(error instanceof Error?error.message:'Could not upload the imported questions.')}finally{setSaving(false)}}
  return <>
