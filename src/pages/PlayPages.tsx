...
import { lookupCloudLiveGame, normalizeGamePin, type LiveGameSnapshot, type LiveJoinCredentials } from '../lib/liveGameRepository'
...
export function PlaySessionPage(){
  ...
  const questionId=quiz?.questionIds[session?.currentQuestionIndex??-1]
  const question=isSupabaseConfigured?snapshot?.questions.find(item=>item.id===questionId):state.questions.find(item=>item.id===questionId)
