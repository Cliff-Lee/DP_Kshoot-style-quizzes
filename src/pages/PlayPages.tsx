...
import { currentQuestionFromSnapshot, isLiveQuestionRevealed, liveCorrectAnswerLabel, lookupCloudLiveGame, normalizeGamePin, type LiveGameSnapshot, type LiveJoinCredentials } from '../lib/liveGameRepository'
...
export function PlaySessionPage(){
  ...
  const revealed=isLiveQuestionRevealed(session)
  ...
  if(revealed)return <div className={`play-page feedback-page ${submitted?(submitted.correct?'correct':'incorrect'):'revealed'}`}><div className="play-texture"/><header><Logo/><span className="score-chip">{participant.score.toLocaleString()} pts</span></header><main>{submitted?(submitted.correct?<CheckCircle2/>:<XCircle/>):<CheckCircle2/>}<span className="play-kicker">{submitted?(submitted.correct?'That’s right':'Not this time'):'Answer revealed'}</span><h1>{submitted?(submitted.correct?`+${submitted.awardedPoints.toLocaleString()} points`:'Keep moving'):'Review the solution'}</h1>{submitted&&<div className="score-breakdown"><span>Base <b>{submitted.scoreDetail.basePoints}</b></span><span>Speed <b>+{submitted.scoreDetail.speedBonus}</b></span><span>Streak <b>+{submitted.scoreDetail.streakBonus}</b></span>{submitted.scoreDetail.multiplier>1&&<span>Powerup <b>×{submitted.scoreDetail.multiplier}</b></span>}{submitted.scoreDetail.shieldUsed&&<span>Shield <b>Streak saved</b></span>}</div>}<section className="student-answer-reveal"><span>Correct answer</span><strong>{liveCorrectAnswerLabel(question)}</strong></section>{question.explanation&&<section className="phone-explanation"><h2>Explanation</h2><p>{question.explanation}</p></section>}<div className="feedback-footer"><span><Flame/>{participant.currentStreak} streak</span><span><Trophy/>#{rank} place</span></div></main></div>
  if(submitted)return <div className="play-page feedback-page locked"><div className="play-texture"/><header><Logo/><span className="score-chip">{participant.score.toLocaleString()} pts</span></header><main><LoaderCircle className="spin"/><span className="play-kicker">Answer locked</span><h1>Eyes on the main screen…</h1><p>Your result and explanation appear when your teacher clicks Reveal Answer.</p></main></div>
